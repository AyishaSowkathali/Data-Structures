...
// definition of RemOddDupEven of Assignment 5 Part 1
void RemOddDupEven(Node*& headPtr) {
	Node *precursor = headPtr; // previous pointer

	while (headPtr != 0 && headPtr->data % 2 != 0) { // Removes odd from begining
		headPtr = headPtr->link;
		delete precursor;
		precursor = headPtr;
	}
	...
	while (cursor != 0) {	// Duplicates even nodes
		if (cursor->data % 2 == 0) {
