...
void HashTable::rehash() {
	...
	size_type newCapacity = next_prime(2 * capacity);

	Item* newData = new Item[newCapacity];
	for (size_type i = 0; i < newCapacity; i++) {
		strcpy(newData[i].word, "");
	}
	used = size();
	data = new Item[capacity];
	capacity = newCapacity;

	for (size_type j = 0; j < oldCapacity; j++) {
		if (strcmp(oldData[j].word, "") != 0) {
			insert(oldData[j].word);
			used--;
		}
	}
	delete[] oldData;
...
bool HashTable::search(const char* cStr) const {
	size_type key = hash(cStr) % capacity;
	size_type index = key;

	for (size_type i = 0; i < capacity; i++) {
		index = (key + (i * i)) % capacity;
		if (!strcmp(data[index].word, cStr))
			return true;
	}
	return false;
}
...
void HashTable::scat_plot(ostream& out) const {
	...
	if (capacity >= 100000)
		...
		width = capacity / 10;
	size_type max_digits = size_type(floor(log10(hi_index)) + 1), label_beg =
			lo_index, label_end = label_beg + width - 1;
	for (label_beg = lo_index; label_beg <= hi_index; label_beg += width) {
		out << endl;
		if (label_end > hi_index)
			out << setw(max_digits) << label_beg << " - " << setw(max_digits)
			<< hi_index << ": ";
		else
			out << setw(max_digits) << label_beg << " - " << setw(max_digits)
			<< label_end << ": ";
...
void HashTable::insert(const char* cStr) {
	...
	for (size_type i = 0; i < capacity; i++) {
		index = (key + (i * i)) % capacity;
		if (strcmp(data[index].word, "") == 0) {
			strcpy(data[index].word, cStr);
			used++;
			i = capacity;
		}
	}
