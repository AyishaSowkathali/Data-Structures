...
int main() {
	int testCasesToDo = 990000, testCasesDone = 0, loOriSize = 1,
			hiOriSize = 30, oriSize, loRemTriesToDo = 1, hiRemTriesToDo,
			remTriesToDo, loValue = -9, hiValue = 9;
	int oriCount, sampCount, remTriesDone, remCountArr, remCountBST, used,
	numNodes, newInt, remInt;
	int *dataOriSorted = 0, *dumpOri = 0, *dumpPostRem = 0, *dataRemRandom = 0,
			*dataRemSorted = 0;
	btNode* bst_root = 0;
	...
	do {
		...
		if (remCountArr != remCountBST || numNodes != used) {
			...
			cout << "expected: " << remCountArr << " removed, " << used
					<< " remained" << endl;
			cout << "got this: " << remCountBST << " removed, " << numNodes
					<< " remained" << endl;
			...
			cout << "from " << oriSize << " values below:" << endl;
			cout << "   ";
			ShowArray(dumpOri, oriSize);
			exit (EXIT_FAILURE);
		} else {
			...
			if (!match(dataOriSorted, dumpPostRem, used)) {
				cout << "Remove error ... bad in-order traversal output"
						<< endl;
				cout << "expected: ";
		...
		if (testCasesDone <= 5 || testCasesDone % 66000 == 0) {
			clog << "testing case " << testCasesDone << " of " << testCasesToDo
					<< endl;
			cout << "==============================" << endl;
			cout << "test case " << testCasesDone << " of " << testCasesToDo
					<< endl;
			cout << "attempt to remove " << remTriesToDo << " values below:"
					<< endl;
			cout << "   (sequential order) ";
			ShowArray(dataRemRandom, remTriesToDo);
			cout << "   (value-sort order) ";
			ShowArray(dataRemSorted, remTriesToDo);
			cout << "from " << oriCount << " values below:" << endl;
			cout << "   ";
