...
void DebugShowCase(int whichCase, int totalCasesToDo, const int caseValues[],
		int caseSize);

int main() {
	int testCasesToDo = 1000000, testCasesDone = 0, loSize = 1, hiSize = 10,
			loValue = 0, hiValue = 9;
	int numInts, used0, used1, intCount, evenCount, newInt, iLenChk1;
	int *intArr0 = 0, *intArr1 = 0;
	Node *head1 = 0;
	...
	do {
		...
		if (!match(head1, intArr1, used1)) {
			...
			cout
			<< "Contents error ... mismatch found in value or order for test_case "
			<< testCasesDone << endl;
			cout << "initial: ";
			ShowArray(intArr0, used0);
			cout << "ought2b: ";
...
void DebugShowCase(int whichCase, int totalCasesToDo, const int caseValues[],
		int caseSize) {
	cout << "test case " << whichCase << " of " << totalCasesToDo << endl;
	cout << "given list: ";
